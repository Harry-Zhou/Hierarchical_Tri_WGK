...
if __name__ == '__main__':
    ...
    path = osp.join('..', 'datasets_finetune_gc')
    ...
    # for method in ['logreg', 'svc', 'linearsvc', 'randomforest']:
    #     acc, f1, roc_auc = evaluate_embedding(emb, y)
    #     accuracies[method].append(acc)
    #     f1_scores[method].append(f1)
    #     roc_auc_scores[method].append(roc_auc)
    ...
    fout = open(osp.join('.', 'unsupervised.csv'), 'w', newline = '', encoding = 'utf-8')
    csv_writer = csv.writer(fout)
    # args.DS, args.num_gc_layers, epochs, log_interval, lr, s
    csv_writer.writerow(['Classifier', \
        'Dataset', \
        'Num_gc_layers', \
        'Epochs', \
        'Learning_rate', \
