...
def create_graph_classification_dataset(dataset_name):
    name = {'reddit_multi_12k': 'reddit_multi_12k', \
        'reddit_multi_5k': 'reddit_multi_5k', \
...
def create_node_classification_dataset(dataset_name):
    dgl_reddit = dgl.data.RedditDataset()
    dgl_cora_full = dgl.data.CoraFullDataset()
    dgl_pubmed = dgl.data.PubmedGraphDataset()
    dgl_citeseer = dgl.data.CiteseerGraphDataset()
    dgl_amazon_cobuy_computer = dgl.data.AmazonCoBuyComputerDataset()
    dgl_mutag = dgl.data.MUTAGDataset()
    dgl_coauthor_cs = dgl.data.CoauthorCSDataset()
    dgl_ppi = dgl.data.PPIDataset()

    return {'reddit': dgl_reddit, \
