...
if __name__ == '__main__':
    ...
    acc, f1, roc_auc = linearsvc(embs, lbls)
    print('Before training, accuracy:  {:.4f}'.format(acc))
    print('Before training,       f1:  {:.4f}'.format(f1))
    print('Before training,  roc_auc:  {:.4f}'.format(roc_auc))
    ...
    acc, f1, auc = linearsvc(embs, lbls)
    print('After training, accuracy:  {:.4f}'.format(acc))
    print('After training,       f1:  {:.4f}'.format(f1))
    print('After training,  roc_auc:  {:.4f}'.format(roc_auc))

    with open(os.path.join('.', 'results', dataset+'_results.csv'), 'w', newline = '', encoding = 'utf-8') as fout:
        csv_writer = csv.writer(fout)
        csv_writer.writerow(['Dataset', \
            'Accuracy', \
            'Micro_F1', \
            'Roc_Auc'])
        csv_writer.writerow([dataset, acc, f1, auc])
