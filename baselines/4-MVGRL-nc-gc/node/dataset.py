...
from dgl.data import CoraGraphDataset, CiteseerGraphDataset, PubmedGraphDataset, RedditDataset, \
    CoraFullDataset, AmazonCoBuyComputerDataset, MUTAGDataset, CoauthorCSDataset, PPIDataset
...
def process_dataset_appnp(dataname, epsilon):
    k = 20
    alpha = 0.2
    if dataname == 'reddit':
        dataset = RedditDataset()
    elif dataname == 'cora_full':
        dataset = CoraFullDataset()
    elif dataname == 'pubmed':
        dataset = PubmedGraphDataset()
    elif dataname == 'amazon_cobuy_computer':
        dataset = AmazonCoBuyComputerDataset()
    elif dataname == 'mutag':
        dataset = MUTAGDataset()
    elif dataname == 'coauthor_cs':
