...
if __name__ == '__main__':
    ...
    for _ in range(5):
        ...
        model.eval()
        logits = model(test_embs).detach().cpu().numpy()
        ...
        if n_classes > 2:
            ...
            roc_auc = roc_auc_score(test_labels, logits, multi_class = 'ovr')
        else:
    ...
    with open(os.path.join('.', 'results', args.dataname+'_results.csv'), 'w', newline = '', encoding = 'utf-8') as fout:
        csv_writer = csv.writer(fout)
        csv_writer.writerow(['Dataset', \
            'Accuracy', \
            'Micro_F1', \
            'Roc_Auc'])
        csv_writer.writerow([args.dataname, \
            np.mean(accuracies), \
            np.mean(f1_scores), \
