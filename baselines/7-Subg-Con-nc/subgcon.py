...
class SugbCon(torch.nn.Module):
    ...
    def test(self, train_z, train_y, val_z, val_y, test_z, test_y, solver='lbfgs',
        ...
        val_z = val_z.detach().cpu().numpy()
        val_pred = clf.predict(val_z)
        val_prob = clf.predict_proba(val_z)
        val_y = val_y.detach().cpu().numpy()
        
        num_cls = val_prob.shape[1]

        test_z = test_z.detach().cpu().numpy()
        test_pred = clf.predict(test_z)
        test_prob = clf.predict_proba(test_z)
        ...
        if num_cls > 2:
            ...
            val_roc_auc = roc_auc_score(val_y, val_pred, multi_class = 'ovr')

            test_f1 = f1_score(test_y, test_pred, average = 'micro')
            test_roc_auc = roc_auc_score(test_y, test_pred, multi_class = 'ovr')
        else:
            val_f1 = f1_score(val_y, val_pred)
            val_roc_auc = roc_auc_score(val_y, val_pred)

            test_f1 = f1_score(test_y, test_pred)
            test_roc_auc = roc_auc_score(test_y, test_pred)
