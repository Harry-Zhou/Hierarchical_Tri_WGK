...
7. 关于超参数，不要硬编码，要用户可以设置，通过验证集可以选择超参数；
8. 深度分析、评估、审核、校验项目中与CSG-Transformer相关的代码，csg_transformer方案中哪些功能没有实现，哪些功能部分实现，哪些功能简化实现。根据分析报告，持续增强、优化、补充、完善相关代码，确保CSG-Transformer相关代码和实验评估方案100%实现，达到production ready。
9. 深度审核、评估、分析、审查、校验Hierarchical_Tri_WGK项目核心代码库（cyclic_schema目录、our_exepriments目录）。评估Hierarchical_Tri_WGK项目核心代码库的实现完成度？哪些没有完成，哪些部分完成，哪些简化完成，哪些还可以继续优化？根据评估报告，增强、补充、修改、完善、优化、升级Hierarchical_Tri_WGK项目核心代码，保证上述任务100%完成，并达到production ready