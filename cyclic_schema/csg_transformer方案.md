...
为使 CSG-Transformer 在图分类和节点分类任务上达到 SOTA 性能，在核心架构之外引入以下六项增强技术。所有增强均通过配置参数控制，不影响基线模型的使用。

#### 6.10.1 相对注意力偏置（GRIT-style Relative Bias）

**动机**：GraphGPS/Graphormer 等模型表明，在注意力分数中加入基于最短路径距离的相对位置偏置可显著提升性能。

**实现**：在 `SparseGlobalAttention` 中，为每个注意力头学习一个距离到偏置的嵌入表 `rel_bias_emb(num_heads, max_dist + 2)`。偏置在 top-k 选择阶段按头均值化使用，在最终注意力计算中按完整多头使用。

**配置参数**：
- `use_rel_bias`（bool，默认 `True`）：是否启用相对偏置
- `max_dist`（int，默认 `5`）：最大编码距离，超出部分截断

#### 6.10.2 虚拟节点（Virtual Node）

**动机**：GraphGPS 提出的虚拟节点技术，通过一个可学习的全局表示节点与所有图节点进行交叉注意力，替代传统池化操作，获得更强的图级表示。

**实现**：在 `CSGTransformer` 中添加可学习参数 `virtual_node`（shape `[1, hidden_dim]`），通过 `nn.MultiheadAttention` 对所有节点嵌入执行交叉注意力，输出经 LayerNorm 残差连接后作为图嵌入。

**配置参数**：
- `use_virtual_node`（bool，默认 `True`）：仅在 `task='graph_classification'` 时生效

#### 6.10.3 DropEdge 正则化

**动机**：DropEdge 在训练时随机删除部分边，防止过平滑和过拟合，在深层 GNN 中尤其有效。

**实现**：在 `train_one_epoch` 中，每个 epoch 对每个图以概率 `drop_edge_rate` 随机删除边，并确保删除后图仍连通（若断开则随机重连各连通分量）。

**配置参数**：
- `drop_edge_rate`（float，默认 `0.0`）：边删除概率，推荐 `0.1-0.3`

#### 6.10.4 DIGL 图重连（Graph Rewiring）

**动机**：异ophilic 图中，同质性低导致消息传递效果差。DIGL 使用 PPR 扩散矩阵识别全局相似节点并添加边，改善消息传递路径。

**实现**：`rewire_graph_diffusion()` 计算 PPR 扩散矩阵 $S = \alpha (I - (1-\alpha) \hat{A})^{-1}$，为每个节点取扩散值最高的 $k$ 个节点添加边（保留原始边）。

**配置参数**：
- `alpha`（float，默认 `0.15`）：PPR 随机游走重启概率
- `k`（int，默认 `2 * ceil(log2(n))`）：每节点新增边数
- `add_original`（bool，默认 `True`）：是否保留原始边

#### 6.10.5 Correct & Smooth 后处理

**动机**：C&S 是一种模型无关的后处理技术，通过标签传播在图上修正和平滑预测，在节点分类任务上可提升 1-5%。

**实现**：
1. **Correct**：计算训练节点上的预测残差，通过归一化邻接矩阵迭代传播（`correct_iter` 次）
2. **Smooth**：将修正后的预测在图上迭代平滑（`smooth_iter` 次），训练节点标签固定

**配置参数**：
- `use_correct_smooth`（bool，默认 `False`）
- `correct_iter`（int，默认 `50`）、`correct_alpha`（float，默认 `0.5`）
- `smooth_iter`（int，默认 `50`）、`smooth_alpha`（float，默认 `0.5`）

#### 6.10.6 自知识蒸馏（Self-KD via EMA Teacher）

**动机**：PolyNormer 等工作表明，使用 EMA 教师模型进行自知识蒸馏可以稳定训练并提升泛化性能。

**实现**：维护模型参数的指数移动平均（EMA）作为教师模型。训练时先用教师模型产生软标签，与学生模型的 KL 散度作为蒸馏损失，与交叉熵损失加权求和。

**配置参数**：
- `use_ema`（bool，默认 `False`）
- `ema_decay`（float，默认 `0.999`）
- `ema_start_epoch`（int，默认 `0`）：EMA 开始 epoch
- `kd_temperature`（float，默认 `4.0`）：蒸馏温度
- `kd_weight`（float，默认 `0.5`）：蒸馏损失权重

#### 6.10.7 增强技术组合推荐

| 任务类型 | 推荐组合 | 预期增益 |
|---------|---------|---------|
| 图分类（小图） | 相对偏置 + 虚拟节点 + DropEdge(0.1) | +1-3% |
| 图分类（大图） | 相对偏置 + 虚拟节点 + DropEdge(0.2) + Self-KD | +2-4% |
| 节点分类（homophilic） | 相对偏置 + C&S | +1-3% |
| 节点分类（heterophilic） | 相对偏置 + DIGL 重连 + C&S | +3-7% |
...
CSG‑Transformer 为图深度学习提供了兼具理论保障与实用性的新方案。CFI 图实验（§6.9）直接验证了理论证明，展示了模型在经典难例上的强大表达力。六项 SOTA 增强技术（§6.10）——相对注意力偏置、虚拟节点、DropEdge、DIGL 图重连、Correct & Smooth、自知识蒸馏——进一步将模型推向图分类和节点分类的 SOTA 水平。未来工作将聚焦于：近似圈基算法以降低构建开销、归纳式设计以扩展应用场景、以及异质图支持以拓宽应用领域。
