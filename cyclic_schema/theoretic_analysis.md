...
$$
K_{\alpha}(G_i, G_j) = \alpha \cdot K_{\text{OT}}(G_i, G_j) + (1 - \alpha) \cdot K_{\text{WL}}(G_i, G_j), \quad \alpha \in [0, 1]
$$
...
$$
\begin{aligned}
K_{\text{OT}}(G_i, G_j) &= \exp\Big(-\gamma \cdot \text{SW}_2^2(G_i, G_j)\Big) \\[4pt]
K_{\text{WL}}(G_i, G_j) &= \frac{\langle \phi(G_i), \phi(G_j) \rangle}{\sqrt{\langle \phi(G_i), \phi(G_i) \rangle \cdot \langle \phi(G_j), \phi(G_j) \rangle}}
\end{aligned}
$$

式中：
- $K_{\text{OT}}$ 是将 Sliced Wasserstein 距离平方 $\text{SW}_2^2$ 经高斯核映射得到的**结构相似度核**（§15）
- $K_{\text{WL}}$ 是 WL 标签计数向量 $\phi(G)$ 经余弦归一化后的**内积核**（线性核）
- $\alpha$ 是平衡系数（通过 `alpha_list` 扫描），控制两者的相对贡献
...
| 维度 | $K_{\text{OT}}$（高斯 OT 核） | $K_{\text{WL}}$（WL 内积核） |
|------|------|------|
...
这等价于测量两个图在多维特征空间中点云的几何差异。即使两个图具有完全相同的标签多重集（标签直方图相同），只要标签在节点上的**空间排列**不同，OT 距离就非零。形式化地，设图 $G_i$ 的 WL 特征向量为 $X_i \in \mathbb{R}^{n_i \times d}$（每行对应一个节点的 $d$ 维标签计数），度分布权重为 $a_i \in \Delta_{n_i}$，则：

$$
\text{SW}_2^2(G_i, G_j) = \mathbb{E}_{\theta} \left[ W_2^2(P_{\theta\#}(X_i, a_i),\; P_{\theta\#}(X_j, a_j)) \right]
$$
...
这是一种集合交集度量。定义 $\phi(G) \in \mathbb{Z}^L$ 为 WL 标签计数向量（$L$ 为标签空间大小），则：

$$
K_{\text{WL}}(G_i, G_j) = \frac{\phi(G_i)^\top \phi(G_j)}{\|\phi(G_i)\| \cdot \|\phi(G_j)\|}
$$
...
| 场景 | $K_{\text{OT}}$ 行为 | $K_{\text{WL}}$ 行为 | 组合效果 |
|------|:----:|:----:|:----:|
...
**定理 16.1**（凸组合的核合法性）。若 $K_{\text{OT}}$ 和 $K_{\text{WL}}$ 均为正定核，则对任意 $\alpha \in [0, 1]$，组合核 $K_{\alpha} = \alpha K_{\text{OT}} + (1-\alpha) K_{\text{WL}}$ 也是正定核。
...
$$
\sum_{i,j=1}^n c_i c_j K_{\alpha}(x_i, x_j) = \alpha \underbrace{\sum_{i,j} c_i c_j K_{\text{OT}}(x_i, x_j)}_{\geq 0} + (1-\alpha) \underbrace{\sum_{i,j} c_i c_j K_{\text{WL}}(x_i, x_j)}_{\geq 0} \geq 0
$$

由 $K_{\text{OT}}$ 和 $K_{\text{WL}}$ 的正定性保证了两项均 $\geq 0$。$\square$

**推论 16.2**（核矩阵的合法性）。对任意 $n \geq 1$ 和 $\alpha \in [0,1]$，核矩阵 $\mathbf{K}_{\alpha} = [K_{\alpha}(G_i, G_j)]_{i,j=1}^n$ 是半正定的。

*意义*：组合核 $K_{\alpha}$ 直接继承了 $K_{\text{OT}}$ 和 $K_{\text{WL}}$ 的核合法性。SVM 等核方法在使用该组合核时，优化问题的凸性（由核矩阵的 PSD 性质保证）得以保持。这意味着 SVM 的理论保证（收敛性、全局最优性）对任意 $\alpha$ 都成立，不需要额外的证明。

**命题 16.3**（核的退化边界）。组合核 $K_{\alpha}$ 在边界取值时退化：

- 当 $\alpha = 0$：$K_0 = K_{\text{WL}}$，完全由 WL 直方图重叠驱动
- 当 $\alpha = 1$：$K_1 = K_{\text{OT}}$，完全由 OT 几何结构驱动
- 当 $\alpha \in (0, 1)$：两者以不同权重贡献

通过扫描 `alpha_list`（通常为 `np.logspace(-2, 0, num=15)`，即 15 个值从 0.01 到 1.0），SVM 的交叉验证自动选择最优 $\alpha$，等价于在"标签直方图驱动"与"几何结构驱动"之间进行软特征选择。
...
$$
\text{Bias}(K_{\alpha}) = \alpha \cdot \text{Bias}(K_{\text{OT}}) + (1-\alpha) \cdot \text{Bias}(K_{\text{WL}})
$$

$$
\text{Var}(K_{\alpha}) \leq \alpha^2 \cdot \text{Var}(K_{\text{OT}}) + (1-\alpha)^2 \cdot \text{Var}(K_{\text{WL}}) + 2\alpha(1-\alpha) \cdot \text{Cov}(K_{\text{OT}}, K_{\text{WL}})
$$

由于 $K_{\text{OT}}$ 和 $K_{\text{WL}}$ 捕捉**正交**的图信息，两者的协方差 $\text{Cov}(K_{\text{OT}}, K_{\text{WL}})$ 预计很小或为负。在这种情况下，组合核的方差低于纯 $K_{\text{OT}}$ 的方差，而偏差低于纯 $K_{\text{WL}}$ 的偏差——这正是集成方法（ensemble methods）的典型优势。

### 16.6 $\alpha$ 值的实际意义

在实验中，`alpha_list` 通常扫描 15 个值 $\{0.01, 0.02, \dots, 0.10, 0.15, \dots, 1.0\}$（对数均匀分布）。不同数据集的交叉验证可能选择不同的最优 $\alpha$，这揭示了该数据集上何种相似度概念更主导：

| 最优 $\alpha$ 区域 | 暗示的数据特性 | 举例 |
|:----:|------|------|
| $\alpha \approx 0$（$\leq 0.05$） | 标签分布差异主导分类；WL 内积已足够 | 标签分布差异大的异质图集 |
| $\alpha \approx 0.5$ | 两者贡献相当 | 大多数中等复杂度数据集 |
| $\alpha \approx 1$（$\geq 0.9$） | 几何拓扑结构主导；标签分布信息不足 | 正则图、强对称图 |

$\alpha$ 的实际默认值设为 0.0（在 `main_topowgk.py` 第 428 行），即仅使用 WL 内积作为快速基准。当需要更高的分类精度时，激活 `alpha_list` 扫描以寻找最优组合。

### 16.7 与 `gk_gamma` 参数的交互

组合核有两个超参数：$\alpha$（平衡系数）和 $\gamma$（高斯核带宽）。两者的交互如下：

- $\gamma$ 控制 $K_{\text{OT}}$ 的"有效半径"：$\gamma$ 越小，$K_{\text{OT}}$ 在所有图上趋近于 1（无区分力）；$\gamma$ 越大，$K_{\text{OT}}$ 趋近于只在非常接近的图对之间非零（过拟合）
- $\alpha$ 在此基础上控制 $K_{\text{OT}}$ 在总核中的比重

实践中，先在固定 $\gamma$ 下扫描 $\alpha$，再在最优 $\alpha$ 附近细化 $\gamma$ —— 形成一个二维超参数搜索空间：

$$
(\alpha^*, \gamma^*) = \arg\max_{\alpha \in [0,1],\; \gamma \in \Gamma} \text{Accuracy}_{\text{CV}}(K_{\alpha, \gamma})
$$

这种**双参数联合搜索**允许核方法同时适应数据集的"结构 vs 特征"偏好（$\alpha$）和"几何尺度"（$\gamma$）。
...
1. **理论合法性**：两个子核均正定，凸组合保持正定性 → 任意 $\alpha$ 下核矩阵合法
2. **信息互补性**：$K_{\text{OT}}$ 捕获几何运输代价（结构差异），$K_{\text{WL}}$ 捕获直方图重叠（特征重叠）→ 两者正交互补
3. **软特征选择**：$\alpha$ 通过交叉验证自动调节结构和特征的相对权重 → 适应不同数据集
