...
**理论贡献概要**：本文建立了从 CSG 变换 $\Phi$ 的代数性质（定理 1.5.3，同构自然性）到核合法性（定理 4.4.2，凸组合 PSD）的完整理论链条。核心创新包括：（a）利用 $\mathbb{F}_2$ 线性无关性证明圈基独占边存在性（引理 2.2.1a）；（b）引入"影响集"论证 HTN-WL 收敛速度（定理 3.6.5）；（c）通过标签等价关系加细证明熵单调性（定理 3.6.8）；（d）证明 HTN-WL 可区分 Shrikhande 图与 Rook 图而 1-WL 和 2-WL 不能（定理 3.10.4）；（e）**对任意 $k \geq 3$ 构造了具体的 CFI 图对，证明 HTN-WL$(L=1)$ 在 MCB 结构差异机制下严格强于 $k$-WL（定理 3.10.5，已通过 $k=3,4,5,6,7$ 实证验证）**；（f）建立 SW² 的 CND → 高斯核 PSD → 凸组合核 PSD 的三步合法性证明链。所有证明均附有严格论证，未证明的猜想（猜想 3.6.11、对标准 CFI 构造的猜想 3.10.7）已明确标注。
...
> **注**（部分解决状态）。本问题在 **简化 CFI 构造**（项目 `our_experiments/cfi_test.py` 中的 vertex clone + edge node 风格）下已被定理 3.10.5 解决：HTN-WL$(L=1)$ 严格强于 $k$-WL。在 **标准 CFI 构造**（深度 $n$ 平衡二叉树 gadget，Cai–Fürer–Immerman 原始论文）下仍未解决，见猜想 3.10.7。
...
> **注**（标准 CFI 的开放性）。Cai–Fürer–Immerman 原始论文使用**深度 $n$ 的平衡二叉树 gadget**（而非本构造的 2-节点 gadget）。在标准 CFI 下，gadget 内部具有高度对称性（树的自同构群），可能导致未扭曲与扭曲版本在 MCB 长度分布上**没有差异**（所有 MCB 元素均为"长圈"，长度 $\gg 6$）。此时 HTN-WL 的 CSG 层无法捕获可区分信号。**对标准 CFI 的推广是真正的开放问题**，留作猜想 3.10.7 的修订版本。
>
...
> - 前者的 MCB 长度分布对扭曲敏感（实证已验证），后者的 MCB 长度分布在两种扭曲下相同（因 gadget 对称性抵消扭曲）
> - 因此本结果**不能**自动推广到标准 CFI。猜想 3.10.7 的修订版精确刻画了剩余的开放问题。
>
...
**猜想 3.10.7**（标准 CFI 下的 HTN-WL 与 $k$-WL 关系，开放问题）。定理 3.10.5 在**简化 CFI 构造**（项目中的 vertex clone + edge node 风格）下证明了 HTN-WL 严格强于 $k$-WL。本猜想询问是否同样的结论对**标准 Cai–Fürer–Immerman 构造**（深度 $n$ 平衡二叉树 gadget）成立：

> 对任意 $k \geq 3$，取 Cai–Fürer–Immerman 原始论文中的标准 CFI 构造（用深度 $n$ 平衡二叉树替换简化 2-节点 gadget），设模板 $H = K_{k+1}$，在某个 $(k-1)$-圈上扭曲，得到图对 $(\mathcal{G}_0, \mathcal{G}_1)$。问：HTN-WL$(L=1, I \geq 1)$ 是否仍能区分 $\mathcal{G}_0$ 与 $\mathcal{G}_1$？

**已知信息**：
- $\mathcal{G}_0 \equiv_{k\text{-WL}} \mathcal{G}_1$（由 Cai–Fürer–Immerman 1992 定理保证，因 $\operatorname{tw}(K_{k+1}) = k$）；
- $\mathcal{G}_0 \not\cong \mathcal{G}_1$（扭曲改变 gadget 内部状态，不同构）；
- 两图的 MCB 长度分布在标准 CFI 下**可能相同**——因深度 $n$ 平衡二叉树 gadget 内部具有高度自同构对称性，扭曲可能被 gadget 内的对称性"吸收"，导致未扭曲与扭曲版本的最小圈基结构完全一致。

**两种可能性**：
- **正面**（猜想为真）：标准 CFI 下 HTN-WL 仍能区分——需证明 CSG 层在 gadget 对称性下仍捕获扭曲信号。**需要新的区分机制**（超出"MCB 长度直方图"），可能涉及 gadget 内节点的标签分布差异。
- **反面**（猜想为假）：标准 CFI 下 HTN-WL$(L=1)$ 失败——因 gadget 对称性使 CSG 节点标签在两种扭曲下完全相同。此时 HTN-WL 需更高 $L$（如 $L=2$）或多尺度机制才能区分。

**为何这是开放问题**：Cai–Fürer–Immerman 原始论文的设计意图是**保护** $k$-WL 不可区分性——即 gadget 越对称越好。HTN-WL 的区分能力来自 MCB 结构差异，而 gadget 对称性恰恰"模糊"了这种差异。这是一场"对称性 vs 拓扑"的博弈，目前未在任何标准 CFI 实例上确定胜方。

> **注**（与定理 3.10.5 的关系）。定理 3.10.5 与猜想 3.10.7 共同刻画了"HTN-WL > $k$-WL"的精确边界：
> - 简化 CFI 构造（定理 3.10.5）：已证明 HTN-WL$(L=1)$ 严格强于 $k$-WL。
> - 标准 CFI 构造（猜想 3.10.7）：开放问题。
>
> 这两个构造的关键差异在于 gadget 的对称性大小。**简化 CFI 的 gadget 不具备"吸收扭曲"的对称性**（因 gadget 小，对称性有限），故扭曲在 MCB 层面可见。**标准 CFI 的 gadget 具备"吸收扭曲"的对称性**（深度 $n$ 平衡二叉树的自同构群阶为 $2^{n} n!$ 量级），故扭曲可能被吸收。
...
**猜想 3.11.3**（标准 CFI 下 HTN-WL 的区分能力，[与猜想 3.10.7 一致]）。对 Cai–Fürer–Immerman (1992) 标准构造的 CFI 图对 $(\mathcal{G}_0, \mathcal{G}_1)$（基于平衡二叉树 gadget），是否存在 $L, I$ 使得 HTN-WL$(L, I)$ 可区分？

**已知部分**：
- **正方向**（猜想为真）：若 gadget 节点在 $\mathcal{G}_0$ 与 $\mathcal{G}_1$ 的"扭曲圈"上**MCB 长度分布不同**（如 §3.10.5 简化 CFI 构造中验证的 $K_4, K_5, K_6, K_7$ 模板），则 HTN-WL$(L \geq 1, I \geq 1)$ 可区分。**实证覆盖范围**：$k$-WL 经验性 $k = 3, 4, 5, 6, 7$ 全部被 $L=1, I=1$ 区分。
...
**研究状态**：开放。CFI 构造下"是否存在 $L^*$ 普适下界使得 $L \geq L^*$ 必区分"是未解决问题。本文 $\Phi$ 变换的非同构保留性（同构自然性，定理 1.5.3）与 gadget 对称性的相互作用仍是活跃研究领域。
...
- 局限：对 Cai–Fürer–Immerman 原始标准 CFI（深度 $n$ 平衡二叉树 gadget）的推广仍为开放问题（猜想 3.10.7）。
...
- **$k \geq 3$（标准 CFI 构造）**：猜想 3.10.7 仍为开放问题。
...
**方向 1a（HTN-WL > $k$-WL 对 $k \geq 3$，已部分解决）**：定理 3.10.5 通过简化 CFI 构造给出了首个此类反例。剩余开放子问题：
- 推广到标准 CFI 构造（猜想 3.10.7）
...
**方向 1b（HTN-WL > $k$-WL 对 $k \geq 3$ 在标准 CFI 下，开放）**：猜想 3.10.7。关键挑战：标准 CFI 的深度 $n$ 平衡二叉树 gadget 内部具有高度对称性（自同构群阶 $\sim 2^n n!$），可能"吸收"扭曲信号，使 $\text{CCB}$ 长度直方图在两种扭曲下相同。
...
> **部分解决状态**（v2 更新）。本问题在**简化 CFI 构造**（项目 `our_experiments/cfi_test.py` 中的 vertex clone + edge node gadget）下已由定理 3.10.5 解决——HTN-WL$(L=1)$ 严格强于 $k$-WL，对 $k=3,4,5,6,7$ 实证验证。**标准 CFI 构造**（深度 $n$ 平衡二叉树 gadget）下仍为开放问题（猜想 3.10.7）。
...
本文的理论分析为 TopoWGK 方法提供了坚实的数学基础，同时揭示了多个值得深入研究的开放问题——特别是**标准 CFI 构造下 HTN-WL 与 $k$-WL 的关系**（猜想 3.10.7 修订版）、$k$-WL 严格强于 HTN-WL 的反例构造（§5.3.1 方向 2）、CSG 变换的谱序列解释（猜想 3.6.11）、以及核方法的泛化界分析——这些方向构成了未来研究的重要议程。
