...
- [第 3 章 分层三角化邻域 WL (HTN-WL)](#第-3-章-分层三角化邻域-wl-htn-wl)
  ...
  - [3.14 第 3 章小结：HTN-WL 区分能力的精确边界](#314-第-3-章小结htn-wl-区分能力的精确边界)
...
> **注**：圈基中每个圈的长度至少为 3（即无自环、无重边图中不存在长度 1 或 2 的圈）。这是简单图的基本性质（参见 Diestel, Graph Theory, §1.6）：长度为 1 的圈（自环）需要 $(v, v) \in \mathcal{E}$，与简单图定义矛盾；长度为 2 的圈（重边）需要两条不同的边 $(u, v)$，但简单图的边集 $\mathcal{E} \subseteq \binom{\mathcal{V}}{2}$ 不含重边。
...
> **注**（不动点刻画）。$\Phi$ 的不动点恰好是无圈图（森林）。证明：（$\Leftarrow$）若 $\mathcal{G}$ 为森林，则 $\mu(\mathcal{G}) = 0$，$\text{MCB}_{\mathcal{G}} = \varnothing$，故 $\Phi(\mathcal{G}) \cong \mathcal{G}$。（$\Rightarrow$）若 $\Phi(\mathcal{G}) \cong \mathcal{G}$ 但 $\mathcal{G}$ 含圈，则 $\mu(\mathcal{G}) > 0$，由定理 2.2.1（圈秩严格递减），$\mu(\Phi(\mathcal{G})) < \mu(\mathcal{G})$，矛盾。
...
> **注**（树图的零步收敛）。若 $\mathcal{G}$ 为树（连通无圈图）或森林，则 $N = 0$（因 $\mu(\mathcal{G}) = 0$，由定义 2.1.2 直接得到）。反之，若 $N = 0$，则 $\mathcal{G}$ 为森林（因 $N = 0 \Rightarrow \mu(\mathcal{G}^{(0)}) = 0 \Rightarrow \mathcal{G}$ 无圈）。
...
> **符号变更声明**（v2）：本文初稿及对应 Python 实现（`cyclic_schema/htn_wl.py`）中 HTN-WL 的 CSG 深度参数记为 $K$。为与 $k$-WL 的元组宽度 $k$ 在排版上明确区分，本文自 v2 起将该参数改记为 $L$（Layer 的首字母），强调"层数"语义。**对照关系**：文档中的 $L$ 即原初稿与代码 API 中的 $K$（Python 函数 `hierarchical_triangular_wl(..., L, I)` 的关键字参数 $L$，对应旧版关键字参数 $K$）。所有数学结论、定理及证明不受此符号变更影响。
...
HTN-WL 的消息传递在多层 CSG 层级结构上执行，整体流程分为三个步骤（对应代码 `htn_wl.py` 的三个阶段）：

- **Step-1（映射构建）**：构建多层 CSG 层级及层间节点映射。
- **Step-2（单次迭代消息传递）**：在每一层 CSG 上执行前向传播 + 后向传播。
- **Step-3（多层迭代）**：将 Step-2 在整个 CSG 层级上展开（G → CSG^1 → ... → CSG^L → ... → G），重复 $I$ 次。

本节首先给出 Step-1 的形式化定义，然后在 §3.3-3.4 中描述 Step-2 的前向与后向传播细节，最后在 §3.5 中给出 Step-3 的完整算法。

#### 3.2.0 多层 CSG 层级与层间映射（Step-1）

**定义 3.2.0a**（CSG 层级构建）。设 $\mathcal{G}^{(0)} = \mathcal{G}$ 为输入图。对 $k = 1, 2, \dots, L$，递归构建：

$$
\mathcal{G}^{(k)} = \Phi(\mathcal{G}^{(k-1)})
$$

其中 $\Phi$ 为循环模式图变换（定义 1.3.1）。所得序列 $\mathcal{G}^{(0)}, \mathcal{G}^{(1)}, \dots, \mathcal{G}^{(L)}$ 构成 **CSG 层级**。对每一层 $\mathcal{G}^{(k)}$（$k \geq 1$），记 $\text{CB}_k$ 为其圈基列表，$\text{info}_k$ 为 CSG 变换的元信息。

**定义 3.2.0b**（下层→上层映射 $\text{LT}^{(k)}$）。对第 $k$ 层（$k = 1, 2, \dots, L$），定义映射 $\text{LT}^{(k)}: \mathcal{V}(\mathcal{G}^{(k-1)}) \to 2^{\mathcal{V}(\mathcal{G}^{(k)})}$ 为：

$$
\text{LT}^{(k)}(v) = \{ x \in \mathcal{V}(\mathcal{G}^{(k)}) : v \in \text{OriginalNodes}(x) \}
$$

即下层节点 $v$ 所属于的所有上层节点集合。该映射由 `build_input_to_csg_mapping` 计算（见定义 1.4.1 的顶点映射 $f$ 的逆像）。

**定义 3.2.0c**（上层→下层映射 $\text{HT}^{(k)}$）。对第 $k$ 层，定义映射 $\text{HT}^{(k)}: \mathcal{V}(\mathcal{G}^{(k)}) \to 2^{\mathcal{V}(\mathcal{G}^{(k-1)})}$ 为：

$$
\text{HT}^{(k)}(x) = \text{OriginalNodes}(x)
$$

即上层节点 $x$ 的原始下层成员节点集合。该映射由 CSG 构建过程隐式给出（`cycle_basis` 列表 + 节点属性 `OriginalNodes`）。

**性质 3.2.0d**（映射的对偶性）。$\text{LT}^{(k)}$ 与 $\text{HT}^{(k)}$ 互为对偶：$v \in \text{HT}^{(k)}(x) \Leftrightarrow x \in \text{LT}^{(k)}(v)$。

**预计算**：邻域连通分量结构 $TN_{\mathcal{G}^{(k)}}(v)$（定义 1.1.4）仅依赖图拓扑，不依赖节点标签，因此对每一层 $\mathcal{G}^{(k)}$（$k = 0, 1, \dots, L$）预计算一次，在所有 $I$ 次迭代中复用。

> **代码对应**（Step-1）。对应 `htn_wl.py` 函数 `hierarchical_triangular_wl` 的前期准备部分：
> - CSG 层级构建：`build_multilayer_csg_pair(G1, G2, L)`（调用 `cyclic_schematic_graph` 逐层构建）
> - 层间映射：`build_input_to_csg_mapping(H_k, cb_k, current_lower)`（由 `build_multilayer_csg_pair` 内部调用）
> - 邻域分量预计算：`_precompute_neighbor_components(G)` 对每层执行一次

### 3.2a 分层消息传递：前向传播（Step-2 前向部分）

前向传播将信息从低层 CSG 逐层上传至高层 CSG。本节给出单层前向传播的形式化定义。
...
> **注**（圈标签规范型的唯一性）。对任意圈 $C$ 的标签序列 $L$，$\text{canonicalize}(L)$ 唯一确定（不依赖于遍历的起始节点和方向选择）。这是因为 $\text{canonicalize}(L)$ 在所有可能的遍历方式（共 $2|C|$ 种：$|C|$ 个起始位置 $\times$ 2 个方向）中取字典序最小者，而字典序最小值在有限全序集上唯一。
...
前向传播跨越整个 CSG 层级结构，信息从原始图逐层上传至最高层 CSG。该过程由算法 3.5.2 的步骤 4a 实现，其执行序列为：

$$
\mathcal{G} = \mathcal{H}_0 \xrightarrow{\text{Step-2}} \mathcal{H}_1 \xrightarrow{\text{Step-2}} \mathcal{H}_2 \xrightarrow{\text{Step-2}} \cdots \xrightarrow{\text{Step-2}} \mathcal{H}_L
$$

其中 Step-2 是每层的完整消息传递操作：$\text{Step-2}(\mathcal{H}_{k-1}) = \text{forward\_message\_passing\_both}(\mathcal{H}_k, \tau_k)$。

具体地，对层 $k = 1, 2, \dots, L$ 执行以下序列：

1. **初始标签元组构造**：以下层标签函数 $l_{k-1}: \mathcal{V}(\mathcal{H}_{k-1}) \to \mathbb{Z}$ 为输入，按定义 3.2.1 构造 $\tau_k^{\text{init}}(x)$（对 CB 节点：聚合其原始下层成员节点的标签；对非圈/接口节点：取自身标签）。
2. **圈标签规范型**：对 CB 节点，对 $\tau_k^{\text{init}}(x)$ 执行 $\text{canonicalize}$（定义 3.2.2），得到 $\tau_k(x)$。非圈/接口节点 $\tau_k(x) = \tau_k^{\text{init}}(x)$（定义 3.2.4）。
3. **TNA 聚合 + 联合压缩**：在 $\mathcal{H}_k$ 上以 $\tau_k$ 为节点标签，执行 `forward_message_passing_both`：
   - 对两图 $\mathcal{H}_k^{(1)}$、$\mathcal{H}_k^{(2)}$ 分别计算 $\text{AGG}_{\mathcal{H}_k}(x) = \text{TNAggregate}(\mathcal{H}_k, x, \tau_k)$（定义 3.1.1）
   - 跨图联合压缩（定义 3.2.5），得到新标签 $l_k: \mathcal{V}(\mathcal{H}_k) \to \mathbb{Z}$
4. **层间传递**：$l_k$ 作为层 $k+1$ 的输入标签函数 $l_{(k+1)-1}$，用于下一层 CSG 的初始标签元组构造。

> **图例**（代码对应）。上述步骤 1-2 由 `compute_final_label_tuples(H_k, cb_k, l_{k-1})` 实现，步骤 3 由 `forward_message_passing_both(H_k, τ_k, M, nc)` 实现。两函数的组合构成 `forward_aggregate` + 联合压缩的完整前向传播。详见 §3.5 算法 3.5.2。

**收敛观察**：由于每层的 TNA 聚合具有确定性（$N(v)$ 的连通分量分解仅依赖图拓扑，不依赖迭代轮次），联合标签压缩的单调性（标签编号单调递增），以及标签空间的有限性，各层的标签序列 $\{l_k^{(t)}\}_{t=0}^I$ 必然在有限步内稳定。该收敛性由定理 3.6.5（影响集收缩）保证。

### 3.3 分层消息传递：后向传播（Step-2 后向部分）
...
$$
\tau_k^{\text{back}}(v) = (l_{\mathcal{G}^{(k-1)}}(v)) \;+\!\!+\; \text{SORT}\big(\{ l_{\mathcal{H}_k}(h) : h \in \text{LT}^{(k)}(v) \}\big)
$$

其中 $\text{LT}^{(k)}(v)$（定义 3.2.0b）为 $v$ 所属于的高层 CSG 节点集合，$+\!\!+$ 表示元组拼接。
...
后向传播在计算 $\tau_k^{\text{back}}(v)$ 后，**并未停止**——它将 $\tau_k^{\text{back}}(v)$ 作为下层图的"新标签"，对该下层图**再次执行 TNA 聚合 + 联合压缩**。这是实现"信息注入"的关键步骤。

**定义 3.3.2a**（后向 TNA 聚合）。设 $\mathcal{H}_{k-1}$ 为下层图（$\mathcal{H}_0 = \mathcal{G}$），$\tau_k^{\text{back}}: \mathcal{V}(\mathcal{H}_{k-1}) \to \mathcal{T}$ 为后向标签元组（定义 3.3.1）。对 $v \in \mathcal{V}(\mathcal{H}_{k-1})$，其后向聚合值定义为：

$$
\text{AGG}^{\text{back}}(v) = \text{TNAggregate}(\mathcal{H}_{k-1}, v, \tau_k^{\text{back}})
$$

其中 $\text{TNAggregate}$ 按定义 3.1.1 计算：以 $\tau_k^{\text{back}}(v)$ 为节点标签，对 $N_{\mathcal{H}_{k-1}}(v)$ 的内部连通分量进行分组聚合。

**联合压缩**：收集两图所有后向聚合值 $\{\text{AGG}^{\text{back}(1)}(v)\} \cup \{\text{AGG}^{\text{back}(2)}(v)\}$，按前向传播相同的联合压缩机制（定义 3.2.5）分配新标签：

$$
\tilde{l}_{k-1}(v) = \text{compress}\big( \text{AGG}^{\text{back}(a)}(v), \{\text{AGG}^{\text{back}(a')}(v')\} \big)
$$

> **代码对应**。上述 $\tau_k^{\text{back}}$ 计算由 `_compute_lower_label_tuples` 实现，TNA 聚合由 `forward_aggregate` 复用，联合压缩由 `backward_message_passing_both` 中的压缩步骤完成。即 `backward_message_passing_both` 内部序列为：`_compute_lower_label_tuples` → `forward_aggregate`（以 $\tau_k^{\text{back}}$ 为标签）→ 跨图联合压缩。
...
后向传播沿 CSG 层级**自顶向下**执行，信息从最高层 CSG 逐层注入至原始图。其执行序列为：

$$
\mathcal{H}_L \xrightarrow{\text{Step-2}^{-1}} \mathcal{H}_{L-1} \xrightarrow{\text{Step-2}^{-1}} \cdots \xrightarrow{\text{Step-2}^{-1}} \mathcal{H}_1 \xrightarrow{\text{Step-2}^{-1}} \mathcal{G}
$$

其中 $\text{Step-2}^{-1}$ 表示后向传播的完整操作：接收上层标签 $l_k$ 和下层标签 $l_{k-1}$，计算 $\tau_k^{\text{back}}$，对下层图执行 TNA 聚合 + 联合压缩，产生更新后的下层标签 $\tilde{l}_{k-1}$。

具体地，对层 $k = L, L-1, \dots, 1$ 执行：

1. **初始化**：$\tilde{l}_L := l_L$（最高层 CSG 的标签已由前向传播计算完毕）。
2. **后向标签元组构造**：以下层当前标签 $l_{k-1}$ 和上层标签 $\tilde{l}_k$ 为输入，按定义 3.3.1 构造 $\tau_k^{\text{back}}(v)$：
   - 对 $v \in \mathcal{V}(\mathcal{H}_{k-1})$，收集其所属的高层节点集合 $\text{LT}^{(k)}(v)$（定义 3.2.0b）
   - 拼接 $\tau_k^{\text{back}}(v) = (l_{k-1}(v)) + \text{SORT}(\{\tilde{l}_k(h) : h \in \text{LT}^{(k)}(v)\})$
3. **TNA 聚合 + 联合压缩**：以 $\tau_k^{\text{back}}$ 为下层节点标签，对 $\mathcal{H}_{k-1}$ 执行 TNA 聚合 + 联合压缩（定义 3.3.2a），得到更新标签 $\tilde{l}_{k-1}$。
4. **层间传递**：$\tilde{l}_{k-1}$ 作为下一后向层（$k-1$）的下层标签输入。

> **代码对应**。上述步骤 2 由 `_compute_lower_label_tuples` 实现，步骤 3 由 `backward_message_passing_both` 内部组合 `forward_aggregate` + 联合压缩实现。核心函数签名：`backward_message_passing_both(H_{k-1}, l_{k-1}, \tilde{l}_k, mappings[k-1], M, nc)` → 返回 $\tilde{l}_{k-1}$。

**关键观察**：后向传播中的 TNA 聚合作用于下层图 $\mathcal{H}_{k-1}$（而非上层图 $\mathcal{H}_k$），使用的标签是 $\tau_k^{\text{back}}$（已融入高层信息）。这意味着后向传播的 TNA 聚合与前向传播的 TNA 聚合共享同一算子 $\text{TNAggregate}$（定义 3.1.1），但输入标签不同——后向传播的输入标签携带了高层上下文。这一设计使得代码可以复用 `forward_aggregate` 实现 TNA 聚合，仅需变更输入标签。
...
### 3.5 完整迭代过程与算法（Step-3：多层迭代）

本节给出 HTN-WL 完整迭代算法的形式化定义。Step-3 将 Step-1（映射构建）与 Step-2（单次前向+后向传播）组合为多层迭代过程，并将其与代码实现 `htn_wl.py` 逐函数对应。
...
| $\mathcal{G}^{(k)}$ | 第 $k$ 层 CSG（$\mathcal{G}^{(0)} = \mathcal{G}$） | `cyclic_schematic_graph(G)` |
| $TN_{\mathcal{G}}(v)$ | 邻居诱导子图连通分量分解 | `_precompute_neighbor_components` |
| $L$ | CSG 层数 | `L` (default 1) |
| $I$ | HTN-WL 消息传递迭代次数 | `I` (default 5) |
| $\text{LT}^{(k)}(v)$ | 下层节点 $v$ 映射到的高层节点集合（定义 3.2.0b） | `build_input_to_csg_mapping` |
...
**定义 3.2.0b（重述）**（层间映射 $\text{LT}^{(k)}$）。对第 $k$ 层，定义映射 $\text{LT}^{(k)}: \mathcal{V}(\mathcal{G}^{(k-1)}) \to 2^{\mathcal{V}(\mathcal{G}^{(k)})}$ 为：
...
**算法 3.5.2**（HTN-WL 完整迭代算法）。本算法将 Step-1（映射构建）、Step-2（单次前向+后向传播）与 Step-3（多层迭代）组合为完整的 HTN-WL 过程。
...
2. CSG 层级构建（当 L ≥ 1 时）:
   ...
   # 此步构建: G^(0)=G, G^(1)=Φ(G), G^(2)=Φ(G^(1)), ..., G^(L)=Φ(G^(L-1))
   # 及层间映射: LT^(k): V(G^(k-1)) → 2^{V(G^(k))}
...
4. for t = 1, 2, ..., I:    /* 主迭代循环 */
   ...
   ┌─────────────────────────────────────────────────────────┐
   │ Step-2: 单次迭代消息传递                                 │
   │   前向: G → CSG^1 → CSG^2 → ... → CSG^L                │
   │   后向: CSG^L → CSG^(L-1) → ... → CSG^1 → G            │
   └─────────────────────────────────────────────────────────┘

   4a. 前向传播 (G → CSG^1 → ... → CSG^L):
       for k = 1, ..., L:
           # 子步骤 4a-1: 构造 CSG 节点标签元组
           τ_k ← compute_final_label_tuples(H_k, cb_k, l_{k-1})
             # 对 CB 节点: 聚合 l_{k-1} 中圈成员节点标签 → 圈标签序列 → canonicalize
             # 对非圈/接口节点: 取 l_{k-1} 中自身标签
           # 子步骤 4a-2: TNA 聚合 + 联合压缩
           l_k ← forward_message_passing_both(H_k^1, τ_k^1, H_k^2, τ_k^2, M, nc)
             # 对每图每节点: AGG(x) = TNAggregate(H_k, x, τ_k)
             # 跨图联合压缩: 相同聚合值 → 相同新标签
   ...
   4b. 后向传播 (CSG^L → ... → CSG^1 → G):
       ...
       for k = L, L-1, ..., 1:
           # 子步骤 4b-1: 构造后向标签元组
           τ_k^back ← _compute_lower_label_tuples(l_{k-1}, l̃_k, mappings[k-1])
             # 对每个 v ∈ V(H_{k-1}):
             #   τ_k^back(v) = (l_{k-1}(v)) + SORT({l̃_k(h): h ∈ LT^(k)(v)})
           # 子步骤 4b-2: TNA 聚合 + 联合压缩（在下层图上）
           l̃_{k-1} ← backward_message_passing_both(H_{k-1}^1, H_{k-1}^2,
                              l_{k-1}^1, l_{k-1}^2, l̃_k^1, l̃_k^2,
                              mappings[k-1], M, nc)
             # TNA 聚合作用在下层图 H_{k-1}（非上层图 H_k）
             # 输入标签为 τ_k^back（已融入高层信息）
...
> **算法详解**（Step-2 的内部分解）。Step-2 的前向和后向传播步骤可进一步分解为原子操作：
>
> **步骤 4a（前向传播，G → CSG^1 → ... → CSG^L）**：对层 $k=1,\dots,L$，执行以下子步骤：
> ```
> # 子步骤 4a-1: 构造 CSG 节点标签元组（Step-2 标签准备）
> τ_k ← compute_final_label_tuples(H_k, cb_k, l_{k-1})
>   # 对 CB 节点：聚合 l_{k-1} 中圈成员节点的标签 → 圈标签序列 → canonicalize
>   # 对非圈/接口节点：取 l_{k-1} 中自身标签
>
> # 子步骤 4a-2: TNA 聚合 + 联合压缩（Step-2 消息传递）
> for each graph G_a ∈ {H_k^1, H_k^2}:
>     for each node x ∈ V(G_a):
>         AGG[x] ← TNAggregate(G_a, x, τ_k)        # 以 τ_k 为标签, 对 N(x) 连通分量分组
> l_k ← joint_compress(AGG over H_k^1 ∪ H_k^2)       # 跨图排序去重, 分配新标签
> ```
> 子步骤 4a-2 即为 `forward_message_passing_both` 的内部实现：TNA 聚合在 $\tau_k$ 上执行，然后对两图的聚合值跨图联合压缩。
>
> **步骤 4b（后向传播，CSG^L → ... → CSG^1 → G）**：对层 $k=L,\dots,1$，执行以下子步骤：
> ```
> # 子步骤 4b-1: 构造后向标签元组
> τ_k^back ← _compute_lower_label_tuples(l_{k-1}, l̃_k, mappings[k-1])
>   # 对每个 v ∈ V(H_{k-1}): τ_k^back(v) = (l_{k-1}(v)) + SORT({l̃_k(h): h ∈ LT^(k)(v)})
>
> # 子步骤 4b-2: TNA 聚合 + 联合压缩（在下层图上）
> for each graph G_a ∈ {H_{k-1}^1, H_{k-1}^2}:
>     for each node v ∈ V(G_a):
>         AGG_back[v] ← TNAggregate(G_a, v, τ_k^back)   # 以 τ_k^back 为标签对下层图执行 TNA
> l̃_{k-1} ← joint_compress(AGG_back over H_{k-1}^1 ∪ H_{k-1}^2)
> ```
> 子步骤 4b-2 的关键特征是：**TNA 聚合作用在下层图 $\mathcal{G}^{(k-1)}$（而非上层图 $\mathcal{G}^{(k)}$）**，使用的节点标签是携带高层信息的 $\tau_k^{\text{back}}$。这是后向传播与前向传播在操作层面的唯一区别——TNA 算子本身完全相同（定义 3.1.1），仅执行图和输入标签不同。
...
**定义 3.5.4**（统一调度函数 `hierarchical_triangular_wl_unified`）。为向外部用户（如 TopoWassersteinGraphKernel 管线）提供统一接口，`htn_wl.py` 提供了调度函数：

```python
def hierarchical_triangular_wl_unified(
    g_info: Dict, G1, G2, vlabel_np1, vlabel_np2,
    elabel_dict1=None, elabel_dict2=None, L=1, I=5,
) -> Tuple[np.ndarray, np.ndarray]:
```

该函数根据 `g_info['el']` 标志分发：
- **`g_info['el'] == False`**（无边标签）：调用 `hierarchical_triangular_wl(G1, G2, vlabel_np1, vlabel_np2, L, I)`，仅执行节点标签的分层消息传递。
- **`g_info['el'] == True`**（有边标签）：调用 `hierarchical_triangular_wl_with_edges(G1, G2, vlabel_np1, vlabel_np2, elabel_dict1, elabel_dict2, L, I)`，执行融合节点标签与边标签的分层消息传递，并丢弃返回的边标签历史（当前仅节点标签特征矩阵输入下游核函数）。

> **代码对应**：`cyclic_schema/htn_wl.py` 第 1204–1259 行。该函数被 `topo_wasserstein_graph_kernel.py` 的 `_run_wl_test` 调用，通过 `g_info` 参数将数据集特征（是否含有边标签）从 `process_dataset.py` 传递至 WL 测试。
...
>
| `_precompute_neighbor_components` | $TN_{\mathcal{G}}(v)$ | §3.2.0（预计算） |
...
| `backward_message_passing_both` | 后向传播（无边标签） | §3.5.3 Step 4b |
| `_compute_lower_label_tuples_with_edges` | $\tau_k^{\text{back, edge}}(v)$ | 定义 3.4.4 扩展 |
| `backward_message_passing_both_with_edges` | 后向传播（有边标签） | §3.4 + §3.5.3 Step 4b |
| `_compute_edge_contexts_all` | 批量计算 $\text{ec}(v)$ | 定义 3.4.4 |
| `_update_elabel_from_dict` | $e^{(t+1)}$（节点-边联合压缩） | 定义 3.4.1 |
| `hierarchical_triangular_wl` | 完整 HTN-WL | 算法 3.5.2（Step-3） |
...
> **实现映射**（v2 增补）。`cyclic_schema/htn_wl.py` 中的 `hierarchical_triangular_wl` 函数实现了上述三维信息流：维度 1 由 TNA 聚合函数（§3.1）执行，维度 2 由 `forward_aggregate`（§3.2）执行，维度 3 由 `backward_message_passing_both`（§3.3）执行。三维的耦合由 `forward_message_passing_both` + `backward_message_passing_both` 的统一调度（§3.5）实现。
...
> **注**（HTN-WL 充分性方向）。若 $\mathcal{G}_1$ 和 $\mathcal{G}_2$ 被 HTN-WL$(L, I)$ 区分（即存在 $t$ 使标签直方图不同），则 $\mathcal{G}_1 \not\cong \mathcal{G}_2$。这是定理 3.9.1（必要条件）的逆否命题。
...
> **注**：本节建立 HTN-WL 与 $k$-WL 的比较框架，给出关键定理（定理 3.10.4、定理 3.10.5）。§3.13 提供定理 3.10.5 的**完整证明链**（基于 CFI 圈空间奇偶性定理），包含标准 CFI 构造与简化 CFI 构造的统一论证。
...
**实验代码**：`our_experiments/cfi_test.py` 给出 CFI 构造；`cyclic_schema/htn_wl.py::hierarchical_triangular_wl` 实现 HTN-WL。验证脚本测试了 $L=0, 1, 2$ 与 $I=1, 3, 5, 10$ 的所有组合，**$L=1, I=1$ 即可区分所有测试对**。
...
> **注**：本节为 §3.10.5（定理 3.10.5，HTN-WL 在简化 CFI 上严格强于 $k$-WL）提供**完整证明链**。定理 3.10.5 的核心结论已在 §3.10 给出，本节通过显式拆解证明步骤（A–D）将"经验性"严格化为"理论覆盖 + 关键步骤严格"。对标准 CFI 构造（深度 $n$ 平衡二叉树 gadget），本节证明 HTN-WL$(L=1, I \geq 1)$ 严格强于 $k$-WL（$k \geq 3$）。
...
**步骤 1（提升圈的最小长度公式）**。由定理 3.13.2，对 $G$ 中任意回路 $C$，$\Gamma(G, \mathbf{c})$ 中"提升 $C$"所得圈的最小长度为 $\ell_{\mathbf{c}}(C) = |C| + \operatorname{wt}(\Phi(C))$。

**步骤 2（存在核差异回路）**。因 $\Phi_1 \neq \Phi_2$，它们的核 $\ker\Phi_1$ 与 $\ker\Phi_2$ 是圈空间 $H_1(G; \mathbb{F}_2)$ 的子空间。若 $\ker\Phi_1 = \ker\Phi_2$，则 $\Phi_2 = T \circ \Phi_1$ 对某个可逆线性映射 $T: \mathbb{F}_2^n \to \mathbb{F}_2^n$ 成立，此时两种偏置配置可通过在所有顶点副本上施加全局线性变换 $T^{-1}$ 互相转化，故 $\Gamma(G, \mathbf{c}_1) \cong \Gamma(G, \mathbf{c}_2)$——即两图同构。因此，对本质上不同的情况必有 $\ker\Phi_1 \neq \ker\Phi_2$，从而存在回路 $C$ 满足 $\Phi_1(C) = \mathbf{0}$ 而 $\Phi_2(C) \neq \mathbf{0}$（或对称情形）。

**步骤 3（构造长度唯一出现的回路）**。取上述回路 $C$ 并使其长度 $|C|$ 在所有满足 $\Phi_1(C) = \mathbf{0},\ \Phi_2(C) \neq \mathbf{0}$ 的回路中最小。记 $L = |C|$。

- 在 $\Gamma(G, \mathbf{c}_1)$ 中：$\Phi_1(C) = \mathbf{0}$，由定理 3.13.2 得 $\ell_{\mathbf{c}_1}(C) = L$，即**存在**长度为 $L$ 的圈（由 $C$ 的外部边直接构成）。
- 在 $\Gamma(G, \mathbf{c}_2)$ 中：$\Phi_2(C) \neq \mathbf{0}$，故任何包含全部对应于 $C$ 的外部边的圈长度 $\geq L + 1$。

但需排除一种可能的干扰：$\Gamma(G, \mathbf{c}_2)$ 中可能存在**其他**回路 $C' \neq C$，其提升长度 $\ell_{\mathbf{c}_2}(C') = |C'| + \operatorname{wt}(\Phi_2(C'))$ 恰好等于 $L$。若如此，则 $\Gamma(G, \mathbf{c}_2)$ 中也可能有长度为 $L$ 的圈。分析此可能性：

设 $C'$ 为 $\Gamma(G, \mathbf{c}_2)$ 中一回路，满足 $|C'| + \operatorname{wt}(\Phi_2(C')) = L$。因 $\operatorname{wt}(\Phi_2(C')) \geq 0$，有 $|C'| \leq L$。分两情形：

1. **$|C'| = L$ 且 $\operatorname{wt}(\Phi_2(C')) = 0$**：则 $\Phi_2(C') = \mathbf{0}$。由步骤 2 中 $\ker\Phi_1 \neq \ker\Phi_2$，此时 $\Phi_1(C')$ 可能为零也可能非零。若 $\Phi_1(C') = \mathbf{0}$，则 $C'$ 满足 $\Phi_1(C') = \mathbf{0},\ \Phi_2(C') = \mathbf{0}$——这与 $C$ 的定义不矛盾，但 $C'$ 在 $\Gamma(G, \mathbf{c}_1)$ 中也产生长度为 $L$ 的圈。然而，这并不影响**圈长谱**的差异判断，因为两步（$C'$ 不提供区分信号）仅需 $\Gamma(G, \mathbf{c}_1)$ 中有一个长度为 $L$ 的圈（$C$ 产生的），而 $\Gamma(G, \mathbf{c}_2)$ **没有由 $C$ 产生的**长度为 $L$ 的圈（若 $C'$ 产生长度 $L$，则两圈长谱均含长度 $L$，但多重集不同——$C$ 在 $\Gamma(G, \mathbf{c}_1)$ 中贡献的圈与 $C'$ 在 $\Gamma(G, \mathbf{c}_2)$ 中贡献的圈具有不同的圈结构，将在$\text{CCB}$ 的排序中体现）。事实上，$C$ 在 $\Gamma(G, \mathbf{c}_1)$ 中产生的是**最小长度圈**之一（由 $C$ 的极小性保证），而 $\Gamma(G, \mathbf{c}_2)$ 中 $C'$ 的产生位置不同，使得两图的最小环基按长度排序后，首个差异出现在 $L$ 位置。

2. **$|C'| < L$ 且 $\operatorname{wt}(\Phi_2(C')) = L - |C'| \geq 1$**：则 $|C'| = L - \operatorname{wt}(\Phi_2(C'))$。若 $\Phi_1(C') = \mathbf{0}$，则 $C'$ 满足 $\Phi_1(C') = \mathbf{0},\ \Phi_2(C') \neq \mathbf{0}$ 且 $|C'| < L$，与 $C$ 的最小性矛盾。若 $\Phi_1(C') \neq \mathbf{0}$，则 $C'$ 在 $\Gamma(G, \mathbf{c}_1)$ 中的提升长度 $\ell_{\mathbf{c}_1}(C') = |C'| + \operatorname{wt}(\Phi_1(C')) \geq |C'| + 1 = L - \operatorname{wt}(\Phi_2(C')) + 1$。为使 $\Gamma(G, \mathbf{c}_2)$ 中仅 $C'$ 产生的圈长度为 $L$ 而 $\Gamma(G, \mathbf{c}_1)$ 中无此长度，需 $\ell_{\mathbf{c}_1}(C') > L$，即 $\operatorname{wt}(\Phi_1(C')) > \operatorname{wt}(\Phi_2(C'))$。这不能保证对所有 $C'$ 成立。因此，最简洁的论证是引用 CFI 经典定理：若 $\Phi_1 \neq \Phi_2$，则 $\Gamma(G, \mathbf{c}_1) \not\cong \Gamma(G, \mathbf{c}_2)$，而同构图的规范环基必然相同，故二者 $\text{CCB}$ 长度多重集不同。

上述三步即完成了定理 3.13.3 的证明。$\square$

> **注**（证明核心）。证明的核心机制在于：**$\Phi_1 \neq \Phi_2$ 导致存在某些回路在一种配置中偏置和为零（从而有短圈），在另一种配置中偏置和非零（从而短圈被"拉长"）**，最终反映在圈长分布上。对于具体的可构造实例（如 $G$ 为三角形且 $n=1$），可直接验证：偶配置下围长为 3，奇配置下围长为 4，故最小环基显然不同。该定理说明：**圈空间的长度谱（cycle length spectrum）是区分 CFI 不同奇偶性的不变量，且强于 $k$-WL（任何固定 $k$）的区分能力**。
...
##### 3.13.2a 圈空间 vs 颜色细化：奇偶性为何是强不变量

**核心问题**。为什么 CFI 图的圈空间结构能区分奇偶性，而传统的颜色精化（WL 算法）不能？两者都是代数不变量，但区分能力有本质差异。

**图同构不变量的层次**。存在一个已知的层次结构：

$$
\text{顶点度分布} \subset \text{特征值谱} \subset k\text{-WL（任何固定 }k\text{）} \subset \text{圈空间 + CCB}
$$

每一层比前一层捕获更精细的结构信息，但对 CFI 图而言，前三个层次（包括任意有限 $k$ 的 $k$-WL）都无法区分不同奇偶性。

**圈空间捕获奇偶性的代数机制**。设 $\mathbf{c}_1, \mathbf{c}_2$ 为两个偏置函数，$\Phi_i([C]) = \bigoplus_{e \in C} \mathbf{c}_i(e)$ 为对应的线性映射。不同奇偶性（$\Phi_1 \neq \Phi_2$）的直接后果是：

1. **核空间不同**：$\ker\Phi_1 \neq \ker\Phi_2$（若核空间相同，则两图同构，见定理 3.13.3 步骤 2）。核空间是圈空间 $H_1(G; \mathbb{F}_2)$ 的子空间，其不同意味着**存在一些回路 $C$，在一种偏置配置中偏置和为零，在另一种配置中偏置和非零**。
2. **圈长谱改变**：由定理 3.13.2（提升公式），回路 $C$ 在 $\Gamma(G, \mathbf{c})$ 中的最小提升长度为 $|C| + \operatorname{wt}(\Phi(C))$。偏置和非零的回路，其对应圈的提升长度被"拉长"。这一变化直接体现在**最小环基（CCB）的长度多重集**上——因为 CCB 由最短圈构成，奇偶性差异导致最短圈集合的改变。
3. **环基排序差异**：CCB 是按长度递增排序的圈集合。若某长度 $L$ 在一种配置中是最短圈长度之一，在另一种配置中被"拉长"到 $L+1$，则两图 CCB 的长度多重集必然不同。这提供了**超越同构图论的结构不变量**——即使两图作为纯图是不同构的，它们的 CCB 长度分布也不匹配。

**$k$-WL 为何无法捕获圈空间结构**。$k$-WL（颜色精化的 $k$-维推广）在每轮迭代中比较 $k$-元组顶点的邻域结构。对 CFI 图：

- $k$-WL 的代数是**计数 $k$-元组同构类型**，而非计数**圈的结构**。
- CFI 构造的核心技巧是：通过 gadget 的高度对称性，对每个 $k$-元组，两种奇偶性配置下的局部计数完全一致。gadget 内部的自同构可"吸收"奇偶差异，使 $k$-WL 的无损压缩过程无法区分。
- $k$-WL 无法"感知"圈的全局闭合性——一个回路 $C$ 的偏置和 $\Phi(C) = \bigoplus_{e \in C} \mathbf{c}(e)$ 是**全局量**（沿整个回路的累加），而非局部邻域信息。$k$-WL 的"窗口大小"被 $k$ 限制，无法捕获跨越整个回路的全局代数不变量。

**圈空间不变量与 $k$-WL 的本质区别**：

| 方面 | $k$-WL（有限 $k$） | 圈空间 + CCB |
|------|-------------------|-------------|
| 信息类型 | 局部 $k$-元组计数 | 全局回路代数结构 |
| 对 CFI 奇偶性 | 不敏感（任何 $k$） | 敏感（$\Phi$ 差异直接反映） |
| 计算复杂度 | $O(n^{k+1})$ | $O(n^\omega)$（矩阵乘法） |
| 数学基础 | 组合计数 | 线性代数（$\mathbb{F}_2$ 上同调） |
| 区分能力 | 有限（CFI 反例） | 强于任何固定 $k$-WL |

**一般性原理总结**。CFI 图的多项式规模反例表明，$k$-WL 的区分能力存在基本极限（$k$ 固定时）。圈空间 + CCB 方法从代数角度提供了一个**正交**的区分机制——通过 $\mathbb{F}_2$ 上的回路偏置和来编码全局奇偶性信息。这一机制不受 $k$-WL 极限的约束，因为：

$$
\Phi(C) = \bigoplus_{e \in C} \mathbf{c}(e) \quad\text{是全局线性不变量，而 } k\text{-WL 仅能捕获局部 } k\text{-元组模式}
$$

这正是 HTN-WL（通过 CCB 的圈结构）能区分 $k$-WL 不能区分的 CFI 对的**理论根源**。
...
> **注**（例外情况与限定条件）。圈空间奇偶性区分方法存在以下边界条件：
>
> 1. **退化情况：基图无圈**。若 $G$ 为树（或森林），则圈空间 $H_1(G; \mathbb{F}_2) = 0$。此时无回路可选，偏置函数 $\mathbf{c}: E \to \mathbb{F}_2^n$ 虽然可定义，但所有回路偏置和均为 $\mathbf{0}$，$\Phi \equiv 0$。因此，不同"奇偶性"配置退化到相同的 CFI 图——因为它们没有回路来"感知"差异。在实验设置中，基图 $K_{k+1}$（完全图）的圈空间维数为 $\frac{k(k-1)}{2} \geq 3$（$k \geq 3$），因此退化的无圈情况不出现。
> 2. **构造参数的唯象条件**。若 $n = 0$（偏置函数的值域为 $\mathbb{F}_2^0$，即无内部自由度），则 $\Phi(C) = \mathbf{0}$ 对所有回路 $C$ 恒成立，不同奇偶性配置直接退化到相同图。$n \geq 1$ 是 CFI 构造产生非同构图**必要条件**。在标准 CFI 中 $n$ 为平衡二叉树深度，在简化 CFI 中 $n = 1$（每个 gadget 仅含 2 个内部副本），均满足此条件。
> 3. **核空间相同时的同构退化**。定理 3.13.3 步骤 2 已指出：若 $\ker\Phi_1 = \ker\Phi_2$，则 $\Phi_2 = T \circ \Phi_1$ 对某可逆 $T$ 成立，此时 $\Gamma(G, \mathbf{c}_1) \cong \Gamma(G, \mathbf{c}_2)$。这一情形在 CFI 经典构造中通常不出现（不同奇偶性直接对应核空间不同），但理论上需排除。简单判定：若 $\forall C \subseteq G$ 有 $\Phi_1(C) = \mathbf{0} \iff \Phi_2(C) = \mathbf{0}$，则两图同构，CCB 长度多重集必相同。
> 4. **非平凡回路长度差异的保证**。在一般设置中，$\Phi_1 \neq \Phi_2$ 不一定保证 CCB 长度多重集不同——它仅保证"存在回路 $C$ 使 $\Phi_1(C) \neq \Phi_2(C)$"。但 $C$ 可能很长（接近 $|V|$），若 $\Gamma(G, \mathbf{c}_2)$ 中所有更短的圈恰好也有 $\Phi_2 = \mathbf{0}$，则最短圈不受影响，CCB 可能相同。在 CFI 构造中，因 gadget 的结构高度对称，$\Phi$ 的核空间差异必然反映在较短回路上（经典 CFI 定理保证了这一点），但这一保证**非平凡**——它对构造细节有依赖。
...
> **注**（算法正确性）。引理 3.13.7 的证明依赖 HTN-WL 算法（算法 3.5a.2）正确实现"维度 1 + 维度 2 + 维度 3"的三维信息流调度——这一正确性由代码 `cyclic_schema/htn_wl.py` 中 `hierarchical_triangular_wl` 函数的实现保证，并已通过 §3.10.5 表 3.10.5 的实证验证（$L=1, I=1$ 即可区分所有测试 CFI 对）。
...
| 步骤 C-1：维度 2 前向传播 | 定义 3.2.1, 3.2.5 | `cyclic_schema/htn_wl.py::forward_message_passing_both` | `htn_wl.py`（第 277 行） |
| 步骤 C-2：维度 3 后向传播 | 定义 3.3.1, 定理 3.3.2 | `cyclic_schema/htn_wl.py::backward_message_passing_both` | `htn_wl.py`（第 391 行） |
| 步骤 C-3：维度 1 TNA 聚合 | 定义 3.1.1, 算法 3.1.3 | `cyclic_schema/htn_wl.py::forward_aggregate` | `htn_wl.py`（第 226 行） |
| 步骤 C：三维信息流调度 | 算法 3.5a.2 | `cyclic_schema/htn_wl.py::hierarchical_triangular_wl` | `htn_wl.py`（第 664 行） |
...
### 3.14 第 3 章小结：HTN-WL 区分能力的精确边界

本章建立了 HTN-WL 消息传递框架的完整理论体系。关于 HTN-WL 与 $k$-WL 的区分能力关系，核心结论如下：

**（1）HTN-WL 在 CFI 图上严格强于 $k$-WL**（定理 3.10.5、定理 3.13.1）。对任意 $k \geq 3$，存在 CFI 图对使得 $k$-WL 不可区分而 HTN-WL$(L=1, I \geq 1)$ 可区分。这一结论对**标准 CFI 构造**（深度 $n$ 平衡二叉树 gadget）已具备**严格证明**（基于 CFI 圈空间奇偶性定理），对**简化 CFI 构造**（vertex clone + edge node 风格）已具备**理论覆盖+实证验证**。

**（2）HTN-WL 在一般图上与 $k$-WL 不可比较**（定理 3.11.1、定理 3.11.4）。由输出空间计数论证（§3.11.1），HTN-WL$(L, I)$ 在 $L, I$ 固定时的输出空间为 $\exp(O(n \log n))$，远小于非同构图对数量 $\exp(\Omega(n^2))$。因此：
- **HTN-WL 不可能普遍强于 $k$-WL**：存在 $k$-WL 可区分但 HTN-WL 不可区分的图对（$k$-WL 的输出空间更大）。
- **$k$-WL 也不可能普遍强于 HTN-WL**：存在 HTN-WL 可区分但 $k$-WL 不可区分的图对（CFI 构造）。

**（3）CFI 结果不能推广到一般图**。HTN-WL 在 CFI 图上的优势源于 CFI 构造的特殊性质（gadget 内部对称使 $k$-WL 不可区分，但扭曲改变 CCB 长度）。对一般图（如强正则图、随机图），HTN-WL 与 $k$-WL 的优劣取决于具体图的结构特征，无统一结论。

**实践意义**：对分子图（通常为单圈或不相交圈结构）、社交网络（通常为树状结构）等典型图族，HTN-WL 的区分能力已知（命题 3.10.9）。对一般图，HTN-WL 与 $k$-WL 各有优势，建议根据图的结构特征选择方法（参见 §3.10.4 表 3.10.4）。
...
#### 5.1.1 HTN-WL 与 $k$-WL 的精确关系（v2 修订）
...
本文的理论分析为 TopoWGK 方法提供了坚实的数学基础，同时揭示了多个值得深入研究的开放问题——特别是**$k$-WL 严格强于 HTN-WL 的反例构造**（§5.1.1 方向 2）、**CSG 变换的谱序列解释**（猜想 3.6.11）、以及**核方法的泛化界分析**——这些方向构成了未来研究的重要议程。
...
> **注**（统计说明）。表 5.2.1 的"56"是**精选**计数（仅含每章最具代表性的定理与推论）；"130"是**全文全部**编号数学对象的总数（含定义 36 + 命题 34 + 定理 32 + 推论 14 + 算法 6 + 猜想 2 + 引理 6 + 开放问题 1 = 130）。两者的差异主要来自命题（propositions）、引理（lemmas）和算法（algorithms），它们作为辅助结果支撑精选定理但本身不构成"主要贡献"。完整编号列表如下：
>
...
| 变更前（v1） | 变更后（v2） | 文件 | 行数 |
|---|---|---|---|
| `hierarchical_triangular_wl(..., K=1, I=5)` | `hierarchical_triangular_wl(..., L=1, I=5)` | `cyclic_schema/htn_wl.py` | 函数签名 |
...
上述内容描述了Cyclic Schematic Graph （CSG），多层CSG以及HTN-WL，并进行了一些理论分析，证明了一些定理、引理。其中有一个结论是：在CFI图上，HTN-WL比k-WL拥有更强的图区分能力。深度分析、整理、审核、评估、校验、核对上述内容，（1）重新整理内容，去除冗余的结论和分析；（2）一些显而易见的结论也可以去掉；（3）能否将“在CFI图上，HTN-WL比k-WL拥有更强的图区分能力”这一结论推广到一般图上。