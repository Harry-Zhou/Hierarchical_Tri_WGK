...
def _parse_filename(fn: str):
    """Parse filenames like: {dataset}_svm_result_n_wl_iters_{wl}_n_csg_layers_{csg}.csv

    Returns (dataset, n_wl_iters, n_csg_layers) or None if not matching.
    """
    m = re.match(
        r'^(?P<dataset>.+)_svm_result_n_wl_iters_(?P<wl>\d+)_n_csg_layers_(?P<csg>\d+)\.csv$',
        fn
    )
    if not m:
        return None
    return m.group('dataset'), int(m.group('wl')), int(m.group('csg'))
...
def aggregate_topowgk_outputs(input_dir: str = TOP_OUTPUT_DIR) -> List[str]:
    ...
    inserts two columns `n_wl_iters` and `n_csg_layers` between `gk_gamma` and `acc`,
    ...
    for dataset, flist in groups.items():
        ...
        for f in sorted(flist):
            ...
            if parsed is None:
                continue
            _, n_wl_iters, n_csg_layers = parsed
            ...
            if 'gk_gamma' in df.columns and 'acc' in df.columns:
                ...
                # insert n_wl_iters right after gk_gamma
                df.insert(gk_idx + 1, 'n_wl_iters', n_wl_iters)
                # insert n_csg_layers after n_wl_iters
                df.insert(gk_idx + 2, 'n_csg_layers', n_csg_layers)
                ...
                df.insert(acc_idx, 'n_wl_iters', n_wl_iters)
                df.insert(acc_idx + 1, 'n_csg_layers', n_csg_layers)
            else:
                # neither column present: append to end
                df['n_wl_iters'] = n_wl_iters
                df['n_csg_layers'] = n_csg_layers
        ...
        # Ensure column order places n_wl_iters and n_csg_layers between gk_gamma and acc if possible
        ...
        if 'gk_gamma' in cols and 'acc' in cols:
            ...
            for c in ['n_wl_iters', 'n_csg_layers']:
                if c in cols:
                    cols.remove(c)
            cols[gk_idx+1:gk_idx+1] = ['n_wl_iters', 'n_csg_layers']
...
def aggregate_svm_results_cli():
    ...
    if args.dataset:
        ...
        matched = [f for f in files if f.startswith(f"{args.dataset}_svm_result_n_wl_iters_hcc_")]
        if not matched:
...
def select_best_priority_and_generate_profiles(processed_dir: str = None, top_output_dir: str = TOP_OUTPUT_DIR) -> str:
    ...
    profile CSVs per selected entry (vary alpha, vary gk_gamma, vary n_csg_layers).
    ...
    out_cols = ['dataset', 'alpha', 'gk_gamma', 'n_wl_iters', 'n_csg_layers',
    ...
    for _, row in best_all.iterrows():
        ...
        alpha = row['alpha']
        gk = row['gk_gamma']
        n_wl_iters = row['n_wl_iters']
        n_csg_layers = row['n_csg_layers']

        # find the source file for (n_wl_iters, n_csg_layers)
        src_fname = f"{ds}_svm_result_n_wl_iters_hcc_({int(n_wl_iters)}_{int(n_csg_layers)}).csv" if (pd.notna(n_wl_iters) and pd.notna(n_csg_layers)) else None
        if src_fname:
        ...
        if not src_path or not os.path.isfile(src_path):
            print(f"Source file not found for {ds} tn={n_wl_iters} hcc={n_csg_layers}: {src_path}; skipping profile generation for this entry")
            continue
        ...
        # (1) fixed gk_gamma, n_wl_iters, n_csg_layers: varying alpha
        if 'gk_gamma' in src_df.columns:
            ...
            try:
                tn_val = int(n_wl_iters)
            except Exception:
                tn_val = n_wl_iters
            try:
                hcc_val = int(n_csg_layers)
            except Exception:
                hcc_val = n_csg_layers
            df_alpha['n_wl_iters'] = tn_val
            df_alpha['n_csg_layers'] = hcc_val
            cols_alpha = ['alpha', 'gk_gamma', 'n_wl_iters', 'n_csg_layers'] + [c for c in metrics if c in df_alpha.columns]
        ...
        # (2) fixed alpha, n_wl_iters, n_csg_layers: varying gk_gamma
        if 'alpha' in src_df.columns:
            ...
            try:
                ...
                hcc_val = n_csg_layers
            df_gk['n_wl_iters'] = tn_val
            df_gk['n_csg_layers'] = hcc_val
            cols_gk = ['alpha', 'gk_gamma', 'n_wl_iters', 'n_csg_layers'] + [c for c in metrics if c in df_gk.columns]
            if not df_gk.empty:
                out2 = df_gk[cols_gk]
        ...
        # (3) fixed alpha, gk_gamma, n_wl_iters: varying n_csg_layers across files
        # find all files for this dataset with same n_wl_iters
        pattern_prefix = f"{ds}_svm_result_n_wl_iters_hcc_({int(n_wl_iters)}_"
        ...
        for mf in sorted(matched_files):
            # extract n_csg_layers from filename
        ...
        if rows_nh:
            ...
            # ensure alpha, gk_gamma, n_wl_iters present
            try:
                tn_val = int(n_wl_iters)
            except Exception:
                tn_val = n_wl_iters
            df_nh['alpha'] = alpha
            df_nh['gk_gamma'] = gk
            df_nh['n_wl_iters'] = tn_val
            cols_nh = ['alpha', 'gk_gamma', 'n_wl_iters', 'n_csg_layers'] + [c for c in metrics if c in df_nh.columns]
            out3_path = os.path.join(proc_dir, f"profile_n_csg_layers_{ds}_svm_results.csv")
            write_header = not os.path.exists(out3_path)
            df_nh.to_csv(out3_path, index=False, mode='a', header=write_header, columns=cols_nh)
            print(f"Appended profile (vary n_csg_layers) {out3_path} ({len(df_nh)} rows)")
